...
public class BookingAddFavoritesTest {
    ...
    public void setFavoritesCheckClolor() throws InterruptedException {
        element = BaseSteps.findElementClickReturn (driver, "//*[@id=\"hotellist_inner\"]/div[1]/div[1]" +
                "/div/button");
        ...
        element = BaseSteps.findElementClickReturn (driver, String.format ("//*[@id=\"hotellist_inner\"]" +
                "/div[%s]/div[1]/div/button", (list.size () - 1)));
        secondHotel = element.getAttribute ("data-hotel-id");
        element = driver.findElement (By.xpath (String.format ("//*[@id=\"hotellist_inner\"]/div[%s]/div[1]" +
                "/div/button/*[1]", (list.size () - 1))));
        TimeUnit.SECONDS.sleep (2);
        Assert.assertEquals ("rgb(204, 0, 0)", element.getCssValue ("fill"));
